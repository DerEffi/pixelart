		...
		server.onNotFound([](AsyncWebServerRequest *request) {
			...
			//Captive portal but no on connected ap only on softAP
				...
				if(sd_connected() && SD.exists(server_home_file)) {
					...
					request->send(400, "text/html", missing_sd);
				}
			} else {
...
void setup() {
	delay(500); //solves issues with too fast starting/restarting resulting in blank panel/artifacts and wifi issues
