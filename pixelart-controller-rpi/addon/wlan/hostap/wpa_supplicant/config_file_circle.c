 ...
 */
static char * wpa_config_get_line(char *s, int size, FIL *stream, int *line,
				  char **_pos)
{
	char *pos, *end, *sstart;

	while (f_gets(s, size, stream)) {
		(*line)++;
		s[size - 1] = '\0';
		pos = s;
...
static struct wpa_ssid * wpa_config_read_network(FIL *f, int *line, int id)
{
...
static struct wpa_config_blob * wpa_config_read_blob(FIL *f, int *line,
						     const char *name)
{
...
static int wpa_config_process_blob(struct wpa_config *config, FIL *f,
				   int *line, char *bname)
{
	char *name_end;
	struct wpa_config_blob *blob;
...
static void write_str(FIL *f, const char *field, struct wpa_ssid *ssid)
{
...
static void write_int(FIL *f, const char *field, int value, int def)
{
	if (value == def)
		return;
	f_printf(f, "\t%s=%d\n", field, value);
...
static void write_bssid(FIL *f, struct wpa_ssid *ssid)
{
...
static void write_psk(FIL *f, struct wpa_ssid *ssid)
{
...
static void write_proto(FIL *f, struct wpa_ssid *ssid)
{
	char *value;
...
static void write_key_mgmt(FIL *f, struct wpa_ssid *ssid)
{
	char *value;
...
static void write_pairwise(FIL *f, struct wpa_ssid *ssid)
{
	char *value;
...
static void write_group(FIL *f, struct wpa_ssid *ssid)
{
	char *value;
...
static void write_auth_alg(FIL *f, struct wpa_ssid *ssid)
{
	char *value;
...
static void write_eap(FIL *f, struct wpa_ssid *ssid)
{
	char *value;
...
static void write_wep_key(FIL *f, int idx, struct wpa_ssid *ssid)
{
	char field[20], *value;
	int res;
...
static void wpa_config_write_network(FIL *f, struct wpa_ssid *ssid)
{
	int i;
...
#ifndef CONFIG_NO_CONFIG_BLOBS
static int wpa_config_write_blob(FIL *f, struct wpa_config_blob *blob)
{
...
static void wpa_config_write_global(FIL *f, struct wpa_config *config)
{
#ifdef CONFIG_CTRL_IFACE
	...
	if (config->dot11RSNAConfigPMKLifetime)
		f_printf(f, "dot11RSNAConfigPMKLifetime=%d\n",
			config->dot11RSNAConfigPMKLifetime);
	if (config->dot11RSNAConfigPMKReauthThreshold)
		f_printf(f, "dot11RSNAConfigPMKReauthThreshold=%d\n",
			config->dot11RSNAConfigPMKReauthThreshold);
	if (config->dot11RSNAConfigSATimeout)
		f_printf(f, "dot11RSNAConfigSATimeout=%d\n",
